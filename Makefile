...
deploy: build push
	...
	@ssh $(NAS_USER)@$(NAS_HOST) "\
		...
		sudo docker run -d -p $(APP_PORT):$(APP_PORT) --name $(PROJECT_NAME) --restart=always $(NAS_HOST):$(REGISTRY_PORT)/$(PROJECT_NAME):latest && \
		echo '' && \
		echo '✅ 部署成功！' && \
...
build:
	...
	@echo "🔨 构建Docker镜像"
	@echo "===================================="
	docker build -f Dockerfile.nginx -t $(NAS_HOST):$(REGISTRY_PORT)/$(PROJECT_NAME):latest .
	@echo "✅ 构建完成"
	@echo ""
