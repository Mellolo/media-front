<template>
  <div class="tag-search-gallery-container">
    <div class="tag-search-gallery-header">
      <h1>标签搜索结果</h1>
    </div>
...
// 获取图集列表
const fetchGalleries = async (tag) => {
  loading.value = true
  try {
    const response = await api.get('/gallery/search/tag', { 
      params: { tag: tag }
    })
...
onMounted(() => {
  if (route.query.tag) {
    ...
    fetchGalleries(route.query.tag)
  } else {
    // 如果没有标签参数，获取所有图集
    fetchGalleries('')
  }
...
.tag-search-gallery-header {
  text-align: center;
  margin-bottom: 40px;
  width: 100%;
}

.tag-search-gallery-header h1 {
  color: #333;
  margin: 0;
  font-size: 28px;
  font-weight: 600;
